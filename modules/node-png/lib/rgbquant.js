...
// image quantizer
RgbQuant.prototype.getReduced32 = function reduce(palette, img, width, dithKern, dithSerp) {
	dithKern = typeof dithKern === 'undefined' ? this.dithKern : dithKern;
	dithSerp = typeof dithSerp !== "boolean" ? this.dithSerp : dithSerp;

	// use palette specified
	this._updatePaletteCache(palette);
	...
	buf32 = data.buf32;
	if (dithKern) {
		buf32 = this.dither(data, dithKern, dithSerp);
	}

	out8 = [];
	for (var i = 0; i < buf32.length; i++) {
		var color = this.rgb(this.nearestColor(buf32[i]));
		out8.push(color[0]);
		out8.push(color[1]);
		out8.push(color[2]);
		out8.push(color[3]);
...
RgbQuant.prototype.getReduced8 = function reduce(palette, img, width, dithKern, dithSerp) {
	dithKern = typeof dithKern === 'undefined' ? this.dithKern : dithKern;
	dithSerp = typeof dithSerp !== "boolean" ? this.dithSerp : dithSerp;
	...
	out8 = [];
	for (var i = 0; i < buf32.length; i++) {
		out8[i] = this.nearestIndex(buf32[i]);
	}
	return out8;
};

RgbQuant.prototype._updatePaletteCache = function(palette) {
	this.idxrgb = [];
	this.idxi32 = [];
...
/**
 * @static
 * @type {{FloydSteinberg: *[], FalseFloydSteinberg: *[], Stucki: *[], Atkinson: *[], Jarvis: *[], Burkes: *[], Sierra: *[], TwoSierra: *[], SierraLite: *[]}}
 */
RgbQuant.kernels = {
	FloydSteinberg     : [
		[7 / 16, 1, 0],
...
RgbQuant.prototype.dither = function (data, kernel, serpentine) {
	if (!kernel || !RgbQuant.kernels[kernel]) {
		throw 'Unknown dithering kernel: ' + kernel;
	}

	var ds = RgbQuant.kernels[kernel];
...
RgbQuant.prototype.buildPal = function buildPal(noSort) {
	...
	switch (this.method) {
		case 1:
			...
			for (var pos = cols; pos < len && histG[sorted[pos]] === freq; pos++) {
...
RgbQuant.prototype.palette = function palette(noSort) {
...
// TODO: igor: no alpha channel support
...
RgbQuant.prototype.rgb = function (i32) {
	return [
			(i32 & 0xff),
			(i32 & 0xff00) >> 8,
			(i32 & 0xff0000) >> 16,
...
RgbQuant.prototype.i32 = function (rgb) {
	return 	((rgb[3] << 24)	|	// alpha
			(rgb[2] << 16)	|	// blue
			(rgb[1] << 8)	|	// green
			rgb[0]);
};
...
var rd      = 255,
	...
	bd      = 255,
	ad      = 255,
	maxDist = Math.sqrt(Pr * rd * rd + Pg * gd * gd + Pb * bd * bd + Pa * ad * ad);

// returns perceptual Euclidean color distance
RgbQuant.prototype.colorDist = function(rgba0, rgba1) {
	var rd = rgba1[0] - rgba0[0],
		gd = rgba1[1] - rgba0[1],
		bd = rgba1[2] - rgba0[2],
		ad = rgba1[3] - rgba0[3];

	return Math.sqrt(Pr * rd * rd + Pg * gd * gd + Pb * bd * bd + Pa * ad * ad) / maxDist;
};
...
function getImageData(rgbaArray, width) {
	var buf8 = new Uint8Array(rgbaArray),
