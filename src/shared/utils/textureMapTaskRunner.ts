...
async function compressTextureMapImage(
  task: InternalTextureMapTask,
  loadedFiles: LoadedFiles,
  ...
  const compressedImages = results.map(result =>
    Buffer.from(result.compressedPNG),
  );
  const compressedImagesSortedByArea = compressedImages.sort(
