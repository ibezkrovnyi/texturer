...
namespace MultiTask {
	...
	class ForkContainer {
		private _task : MasterTask;
		private _online : boolean;
		private _worker : any;
		private _callback : (forkContainer : ForkContainer, task : MasterTask, error : string, data) => void;
		private _thisArg : Object;
		private _tasksFolder : string;

		constructor(tasksFolder : string, callback : (forkContainer : ForkContainer, task : MasterTask, error : string, data) => void, thisArg : any) {
	...
	export class Master {
		private _shutdown : {callback : () => void; scheduled : boolean};
		private _tasks : MasterTask[];
		private _availableForkContainers : { [workerTaskFile : string] : ForkContainer[] };
		private _busyForkContainers : { [workerTaskFile : string] : ForkContainer[] };
		...
		private _onMessage(forkContainer : ForkContainer, task : MasterTask, error : string, taskResult) {
		...
		private _getAvailableForkContainer(task : MasterTask) {
			const taskName = task.getFile();
		...
		private _prepareForTask(task : MasterTask) : void {
			const taskName = task.getFile();
