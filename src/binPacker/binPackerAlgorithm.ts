namespace Texturer {
	...
	export class BinPacker {
		...
		private static _recursiveFindPlace(node : binPackerNode, width : number, height : number) : { x : number; y : number; } {
			if (node.leftChild) {
		...
		placeNextRectangle(width : number, height : number) : {x : number; y : number} {
